...
class OnClassCondition extends FilteringSpringBootCondition {
    ...
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        if (onClasses != null) {
            // @Conditional注解实现中，条件过滤的核心方法，其实就是Class.forName一下ConditionalOnClass注解中的类，如果有异常说明上下文中没这个类，则返回false
            ...
            matchMessage = matchMessage.andCondition(ConditionalOnClass.class).found("required class", "required classes").items(Style.QUOTE, filter(onClasses, ClassNameFilter.PRESENT, classLoader));
        }
        ...
        if (onMissingClasses != null) {
            ...
            matchMessage = matchMessage.andCondition(ConditionalOnMissingClass.class).didNotFind("unwanted class", "unwanted classes").items(Style.QUOTE, filter(onMissingClasses, ClassNameFilter.MISSING, classLoader));
        }
